...
HTMLそのものは保存・送信しない。編集可能領域内の選択文字列も除外する。さらに、カード番号、パスワード、APIキー、認証トークンらしい文字列を正規化時にマスクする。URLの機密クエリ値は`[REDACTED]`へ置換し、username/passwordが埋め込まれたURLは拒否する。タイトルとmeta descriptionも同じ秘匿化処理を通す。共通ZodスキーマでURL、日時、各文字数上限を検証し、本文は最大50,000文字へ切り詰める。本文が空でも、タイトルやメタ情報による後続分析を妨げない。
...
- `?access_token=test-secret`を含むURLで秘密値が`[REDACTED]`相当へ置換される。
